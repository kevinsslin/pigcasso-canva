...
type SpaceModulesPanelProps = {
  ...
  onAddModule: (definition: SpaceModuleDefinition) => void;
};

export const SpaceModulesPanel = ({ modules, onAddModule }: SpaceModulesPanelProps) => {
  const [query, setQuery] = useState("");
  const [activeCategory, setActiveCategory] = useState<"All" | SpaceModuleDefinition["category"]>("All");

  const selectedCategoryModules = useMemo(() => {
    if (activeCategory === "All") return modules;
    return modules.filter((definition) => definition.category === activeCategory);
  }, [activeCategory, modules]);

  const filteredModules = useMemo(() => {
    const normalized = query.trim().toLowerCase();
    if (!normalized) return selectedCategoryModules;

    return selectedCategoryModules.filter((definition) => {
  ...
  }, [query, selectedCategoryModules]);

  const groupedModules = useMemo(() => {
    const groups = new Map<SpaceModuleDefinition["category"], SpaceModuleDefinition[]>();
    for (const definition of filteredModules) {
      const list = groups.get(definition.category) ?? [];
      list.push(definition);
      groups.set(definition.category, list);
    }

    return (["Basics", "Web3"] as const)
      .map((category) => ({ category, modules: groups.get(category) ?? [] }))
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-white/60 bg-white/70 shadow-soft backdrop-blur">
      <div className="border-b border-white/60 px-4 py-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-1 rounded-full border border-white/60 bg-white/70 p-1 shadow-soft">
          {(["All", "Basics", "Web3"] as const).map((category) => (
            <button
              key={category}
              type="button"
              className={cn(
              ...
              <div className="space-y-2">
                {group.modules.map((definition) => (
                  <button
                    key={definition.type}
                    type="button"
                    onClick={() => onAddModule(definition)}
                    draggable
                    onDragStart={(event) => setSpaceModuleDragData(event.dataTransfer, definition.type)}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary ring-1 ring-black/5 transition-colors group-hover:bg-cyan-400/10 group-hover:text-cyan-700">
                        <definition.icon className="size-5" />
                      </div>

                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <div className="truncate text-sm font-bold text-gray-900">{definition.label}</div>
                        ...
                        <div className="mt-1 line-clamp-2 text-xs text-muted-foreground">{definition.description}</div>
                        <div className="mt-2 text-[10px] font-semibold text-muted-foreground">
                          Default size: {definition.defaultLayout.w}×{definition.defaultLayout.h}
                        </div>
                      </div>
                    </div>
                  </button>
