...
import { MintProgressView } from "@/features/editor/components/export-nft-dialog/components/mint-progress-view";
import { getFactoryAddress, isEvmAddress } from "@/features/editor/components/export-nft-dialog/lib/evm";
import { isUserRejectedWalletAction } from "@/features/editor/components/export-nft-dialog/lib/is-user-rejected-wallet-action";
import { makeWorkspacePngDataUrl } from "@/features/editor/components/export-nft-dialog/lib/make-workspace-png-data-url";
import type { ExportedAsset, MintStepKey, MintStepsState, MintView } from "@/features/editor/components/export-nft-dialog/lib/types";
import { getInitialMintSteps } from "@/features/editor/components/export-nft-dialog/lib/types";
      ...
      <DialogContent className="sm:max-w-[520px]">
        ...
        {mintView === "progress" ? (
          <MintProgressView
            ...
            pageLabel={activePage ? `Page ${activePage.index + 1}` : "Select a page"}
            mintStepList={mintStepList}
            mintSteps={mintSteps}
            previewUrl={previewUrl}
            tokenUriForDisplay={tokenUriForDisplay}
            imageUrlForDisplay={imageUrlForDisplay}
            mintResult={mintResult}
            mintError={mintError}
            isMinting={isMinting}
            canRetry={!isMinting && !mintResult && Boolean(mintError)}
            onBack={() => {
              if (isMinting) return;
              if (mintResult) {
                onOpenChange(false);
