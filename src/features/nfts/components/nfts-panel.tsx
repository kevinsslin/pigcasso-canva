...
export const NftsPanel = ({ enabled = true, className }: NftsPanelProps) => {
  const assets = useListNftAssets(undefined, { enabled });
  const collections = useListNftCollections(undefined, { enabled });
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <div className="truncate font-medium">
                          {asset.name ??
                            `${asset.projectName}${asset.pageIndex !== null ? ` · Page ${asset.pageIndex + 1}` : ""}`}
                        </div>
                      ...
                      {asset.collectionAddress ? (
                        <div className="mt-2 text-[11px] text-muted-foreground font-mono">
                          {shortHash(asset.collectionAddress)} {asset.tokenId ? `#${asset.tokenId}` : ""}
                        </div>
                      ) : null}
                    </div>
                  </div>
