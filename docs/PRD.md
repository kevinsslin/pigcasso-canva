...
- Generate Image / Remove BG：`GEMINI_IMAGE_MODEL`（預設 `imagen-3.0-generate-002`）
...
- `GEMINI_IMAGE_MODEL=imagen-3.0-generate-002`（可選）
...
- AI provider：Gemini-only（`imagen-3.0-generate-002`）
